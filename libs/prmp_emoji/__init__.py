...
class Base:
    ...
    @property
    def _subs(self): return self.__subs

    @property
    def subs(self): return list(self._subs.values())
    
    @property
    def subs_names(self): return list(self._subs.keys())

    @property
    def subs_count(self): return len(self._subs)
...
class Common(Base):
    ...
    def __str__(self): return f'{self.name}(master={self.master.name}, subs={self.subs_count})'
...
class Emoji_Base(Common):
    ...
    def __lt__(self, other): return self.hexcode < other.hexcode
    def __gt__(self, other): return self.hexcode >= other.hexcode
    def __le__(self, other): return self.hexcode <= other.hexcode
    def __ge__(self, other): return self.hexcode >= other.hexcode
    
    @property
    def emoji(self): return self.hexcode_to_emoji(self.hexcode)
    
    @property
    def png(self): return self.image.replace('.SVG', '.PNG')

    @property
    def svg(self): return self.image.replace('.PNG', '.SVG')
    
    @property
    def png2(self): return self.non_qualified + '.png'

    @property
    def svg2(self): return self.non_qualified + '.svg'
...
class Emoji_Variation(Emoji_Base):
    ...
    def __str__(self): return f'{self.name}_{self.skintone}(master={self.master.name}, subs={self.subs_count})'
    ...
    @property
    def skintone_base_emoji(self): return self.master
    
    @property
    def skintone_base_hexcode(self): return self.master.hexcode
    
    @property
    def tags(self): return self.master.tags

    @property
    def subgroup(self): return self.master.subgroup

    @property
    def group(self): return self.subgroup.group
...
class Emoji(Emoji_Base):
    ...
    def __init__(self, master, name, **kwargs):
        ...
        self.tags = []
        self.NAME = ''
        if kwargs: self.update_data(**kwargs)
    
    @property
    def all_variations(self): return [self.hexcode, *self.variations_hexcode]
    ...
    @property
    def emojis(self): return list(map(self.hexcode_to_emoji, self.all_variations))
    ...
    def subdict(self):
        ...
        if self._variations:
            variations = {}
            for var in self.variations: variations[var.name] = var.subdict
    ...
    @property
    def _variations(self): return self._subs
    @property
    def variations(self): return self.subs
    @property
    def variations_names(self): return self.subs_names
    @property
    def variations_hexcode(self): return [var.hexcode for var in self.variations]
    ...
    def update_data(self, data, jsonfile):
        ...
        if jsonfile == 'e':
            ...
            assert group == self.group.name and subgroup == self.subgroup.name, 'Error group or subgroup is incorrect!'
            # grouping
            ...
            if (skintone_base_hexcode != hexcode) and (skintone_base_hexcode in [self.hexcode, self.non_qualified]):
                ...
                if tags: self.tags.extend(tags.split(', '))
        else:
            del data['emoji']
            vars = data.get('variations')
            if vars: del data['variations']
            self.__dict__.update(data)
            self.__dict__['image'] = self.__dict__['image'].upper()
    ...
    @property
    def subgroup(self): return self.master
...
class Emoji_Sub_Group(Common):
    ...
    def subdict(self):
        ...
        del dic['name']
        if self._emojis:
            for emoji in self.emojis: dic[emoji.name] = emoji.subdict
        return dic
    ...
    @property
    def group(self): return self.master

    @property
    def _emojis(self): return self._subs
    @property
    def emojis(self): return self.subs
    @property
    def emojis_names(self): return self.subs_names
    @property
    def emojis_count(self): return self.subs_count
    
    def get_emoji(self, name='', hexcode='', non_qualified=''):
        if name:
            name = self.normalize_name(name)
            if name in self.emojis_names: return self._emojis[name]

            for emoji in self.emojis_names:
                if name in emoji: return self._emojis[emoji]
        
        elif hexcode:
            for emoji in self.emojis:
                if (hexcode in emoji.variations_hexcode) or (hexcode == emoji.hexcode): return emoji
                    # get variations

        elif non_qualified:
            for emoji in self.emojis:
                if non_qualified == emoji.non_qualified: return emoji
...
class Emoji_Group(Common):
    ...
    def __init__(self, master, name, data):
        ...
        for sub_group_name, sub_group_data in data.items():
            ...
            self._subs[sub_group_name] = Emoji_Sub_Group(self, sub_group_name, sub_group_data)
    ...
    @property
    def _subgroups(self): return self._subs
    @property
    def subgroups(self): return self.subs
    @property
    def subgroups_names(self): return self.subs_names
    @property
    def subgroups_count(self): return self.subs_count

    @property
    def emojis_count(self): return sum([group.emojis_count for group in self.subgroups])
    
    @property
    def emojis(self):
        _emos = []
        for subgroup in self.subgroups: _emos.extend(subgroup.emojis)
        return _emos
    
    def get_subgroup(self, name, exact=0):
        name = self.normalize_name(name)
        if name in self.subgroups_names: return self._subgroups[name]

        for subgroup in self.subgroups_names:
            if exact: valid = name == subgroup
            else: valid = name in subgroup
            
            if valid: return self._subgroups[subgroup]
...
class EMOJIS(Base):
    ...
    def __init__(self):
        ...
        for emoji in self.emojis:
            ...
            for tag in emoji.tags:
                if tag not in self.emoji_names: self.emoji_names[tag] = emoji

    def __setstate__(self, dict):
        self.__dict__.update(dict)
        global PRMP_EMOJI_DICT
        if not PRMP_EMOJI_DICT: PRMP_EMOJI_DICT = json.dumps(self.dict)
    ...
    @property
    def _groups(self): return self._subs

    @property
    def groups(self): return self.subs
    @property
    def groups_names(self): return self.subs_names

    @property
    def groups_count(self): return self.subs_count

    @property
    def subgroups_count(self): return sum([group.subgroups_count for group in self.groups])
    
    @property
    def emojis_count(self): return sum([group.emojis_count for group in self.groups])

    def get_group(self, name, exact=0):
        name = self.normalize_name(name)
        if name in self.groups_names: return self._groups[name]

        for group in self.groups_names:
            if exact: valid = name == group
            else: valid = name in group
            
            if valid: return self._groups[group]
    ...
    def parse_jsons(self):
        if PRMP_EMOJI_DICT:
            ...
            for emoji in OPENMOJI_DICT:
                if 'extra' in emoji.get('group'): continue
                ...
                emo: Emoji = func(hexcode=hexcode) or func(non_qualified=hexcode) or func(hexcode=base_hexcode) or func(non_qualified=base_hexcode)
                
                if emo: emo.update_data(emoji, 'o')

        save_emoji(self, '_prmp_emoji.obj')

    def to_json(self, file=''):
        if PRMP_EMOJI_DICT: return
