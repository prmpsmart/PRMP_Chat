...
class User(_User):
    ...
    def data(self) -> Tag:
        ...
        for user in self.users.objects:
            users[user.id] = Tag(name=user.name, icon=user.icon)

        for group in self.groups.objects:
            groups[group.id] = Tag(
                name=group.name,
                icon=group.icon,
        ...
        for channel in self.channels.objects:
            channels[channel.id] = Tag(
                name=channel.name,
                icon=channel.icon,
                creator=channel.creator.id,
                admins=channel.admin_ids,
                users=channel.ids,
            )
...
class Multi_Users(_Multi_Users):
    ...
    def remove_user(self, user: User, admin: User):
        if admin.id not in self.admin_ids:
            return RESPONSE.FAILED
        ...
        while True:
            ...
            if action == ACTION.SIGNUP:
                ...
                if response == RESPONSE.EXIST:
                    ...
                    if user.id in USERS_SESSIONS or user.status == STATUS.ONLINE:
...
class Session_Parser:
    ...
    def queued(self, tag):
        ...
        if queued:
            for chat in queued:
                res = self.session.client.send_tag(chat)
    ...
    def create(self, tag: Tag) -> Tag:
        ...
        if type in [TYPE.ADMIN, TYPE.CONTACT]:
            response = RESPONSE.FAILED
        else:
            top_manager = MANAGERS[type]
            manager = self.managers[type]

            response = top_manager.create(
                id=id, name=name, icon=icon, creator=self.user
            )
            if response == RESPONSE.SUCCESSFUL:
                manager.add(top_manager.OBJS[id])
        tag.response = response
        return tag
    ...
    def change(self, tag: Tag) -> Tag:
        ...
        if change not in [TAG.ID, TAG.NAME]:
            return RESPONSE.FAILED
        else:
            top_manager = MANAGERS[type]
