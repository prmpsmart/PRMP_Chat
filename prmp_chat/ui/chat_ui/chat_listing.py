...
class ChatWidget(QFrame):
    def __init__(self, chat_object: Chats, callback=None, index=0):
        ...
        self.unread_bubble.setStyleSheet(
            "background: green; border-radius: 5px; color: white"
        )
    ...
    def enterEvent(self, event):
        if not self.isCurrent:
            self.setStyleSheet(self._style % (STYLE.LIGHT, STYLE.DARK))

        self.icon.setStyleSheet("background: %s" % STYLE.LIGHT_SHADE)

    def leaveEvent(self, event=0):
        if not self.isCurrent:
            self.setStyleSheet(self._style % (STYLE.DARK_SHADE, STYLE.LIGHT))
    ...
    def showEvent(self, event):
        ...
        self.date.setGeometry(
            w - self.dbr.width() - 6, 0, self.dbr.width(), self.dbr.height()
        )

        if self.chat_object.unread_chats:
            self.unread_bubble.setGeometry(
                w - self.ubr.width() - 12,
                (h - self.ubr.height()) / 2,
                self.ubr.width() + 6,
                self.ubr.height() + 3,
            ...
            self.unread_bubble.hide()

        self.time.setGeometry(
            w - self.tbr.width() - 6, 48, self.tbr.width(), self.tbr.height()
        )
...
class ChatsList(ScrolledWidget):
    ...
    def callback(self, chat_widget: ChatWidget, q=1):
        ...
        if q:
            self._callback(chat_widget.chat_object)
        self.current.check()
    ...
    def set_current_object(self, chat_object):
        ...
        if widget:
            self.callback(widget, q=0)

    def update_subs(self, func: str):
        for v in self._chats_objects_.values():
            getattr(v, func)()

    def update_statuses(self):
        self.update_subs("update_status")
...
class SearchList(ChatsList):
    ...
    def search(self, text):
        ...
        for co in self.chats_objects:
            if text in co.name.lower() or text in co.id.lower():
