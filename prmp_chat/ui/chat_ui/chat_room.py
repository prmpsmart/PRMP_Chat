        ...
        if tag == "img":
            for x in attrs:
                if x[0] == "src":
                    ...
                    self._textout += ":{}:".format(
                        x[1].split(os.path.sep)[-1].split(".")[0]
                    )
        elif tag == "p":
            self.paragraph = True
...
class ChatRoomButton(IconButton):
    ...
    def enterEvent(self, event):
        self.setStyleSheet(
            "background: %s; color: %s" % (STYLE.DARK_SHADE, STYLE.LIGHT)
        )
...
class ChatHeader(ChatRoomH_F):
    def __init__(self, chat_object, close):
        ...
        self.icon.setStyleSheet(
            "text-align: center; background: %s" % STYLE.LIGHT_SHADE
        )
        ...
        s, w = 30, 40
        self.phone_call = ChatRoomButton(
            icon="chat_room/phone-call.svg", parent=self, size=w
        )

        self.video_call = ChatRoomButton(
            icon="chat_room/video.svg", parent=self, size=w
        )
...
class TextInput(QTextEdit):
    def __init__(self, parent=None, callback=None):
        ...
        self.setStyleSheet(
            "background: %s; color: %s; padding: 3px"
            % (STYLE.LIGHT_SHADE, STYLE.DARK_SHADE)
        )
...
class ChatFooter(ChatRoomH_F):
    def __init__(self, emoji, room=None):
        ...
        self.emoji = ChatRoomButton(
            parent=self._wid, icon="chat_room/mood-smile.svg", size=self.w
        )
        ...
        self.link = ChatRoomButton(
            parent=self._wid, icon="chat_room/link.svg", size=self.w
        )

        self.send = ChatRoomButton(
            parent=self._wid, icon="chat_room/send.svg", size=self.w
        )
    ...
    def restructure_line(self, line):
        ...
        for ln in range(lines):
            txt = (
                line[
                    ln
                    * self.max_letters_per_line : (ln + 1)
                    * self.max_letters_per_line
                ]
                + "\n"
            )
            text += txt
    ...
    def setup(self):
        ...
        if self.raw_text:
            ...
            if self.lines > 5:
                ...
                hh = abs(
                    self.textView.fontMetrics()
                    .boundingRect(
                        0, 0, self.textView.width(), -1, Qt.TextWrapAnywhere, text
                    )
                    .height()
                )
            ...
            wbr = (
                self.textView.fontMetrics()
                .boundingRect(0, 0, 0, 0, 0, self.longest_line)
                .width()
            )
...
class ImageType(MessageType):
    ...
    def init(self):
        ...
        self.scaled = (
            self._image.scaled(w, h, Qt.KeepAspectRatio, Qt.SmoothTransformation)
            if sc
            else self._image
        )
...
class ChatMessage(QFrame):
    ...
    def defaults(self):
        ...
        self._wid, self._layout = SETUP_FRAME(
            self.lay, re_obj=1, margins=[4, 4, 4, 4], space=2
        )

        self.maxw = self.mst.width() * 0.6
        ...
        if chat == CHAT.TEXT:
            self.messageFrame = TextType(tag=self.tag, maxw=self.maxw)
        elif chat == CHAT.AUDIO:
            self.messageFrame = AudioType(tag=self.tag, maxw=self.maxw)
        elif chat == CHAT.IMAGE:
            self.messageFrame = ImageType(tag=self.tag, maxw=self.maxw)
        elif chat == CHAT.VIDEO:
            self.messageFrame = VideoType(tag=self.tag, maxw=self.maxw)
        else:
            self.messageFrame = MessageType()

        if isinstance(self.chat_object, Contact):
            self.chatType = 1
        elif isinstance(self.chat_object, Group):
            self.chatType = 2
        elif isinstance(self.chat_object, Channel):
            self.chatType = 3
    ...
    def arrange_widgets(self):
        self.timeFrame, self.timeFrameLayout = SETUP_FRAME(
            self._layout, orient="h", re_obj=1, space=3
        )
        ...
        if self._sender:
            ...
            if self.chatType == 2:
                ...
                self.iconLabel.setStyleSheet(
                    "text-align: center; border-radius: 20px; background: %s"
                    % STYLE.LIGHT_SHADE
                )
    ...
    def setup(self):
        if self.chatType == 2 and not self._sender:
            chat_object = self.user.groups.get(
                self.tag.sender
            ) or self.user.contacts.get(self.tag.sender)

            if chat_object and chat_object.icon:
                icon = chat_object.icon
            else:
                icon = None
    ...
    def update_(self):
        if self.statusLabel:
            if self.tag.sent:
                icon = "chat_room/check.svg"
            else:
                icon = "chat_room/clock.svg"
...
class ChatTab(QTabWidget):
    def __init__(self, client: Client, profile, send_back):
        ...
        # if self.user:
        #     self.add_chat_room(self.user.contacts["ade1"])

    def add_chat_room(self, chat_object):
        id = chat_object.id
        if id in self._chats_index_:
