...
class Label(QLabel):
    def __init__(self, ps=0, bold=0, underline=0, layout=None):
        ...
        if layout:
            layout.addWidget(self, Qt.AlignCenter)
...
class Profile(QFrame):
    ...
    def showEvent(self, e):
        ...
        if icon:
            if MASK:
                icon = GET_ICON(self.user.icon, size=200)
            else:
                icon = QIcon(QPixmap(QImage().fromData(b64decode(self.user.icon))))
    ...
    def update_details(self):
        ...
        if self.user.status == STATUS.ONLINE:
            text = str(STATUS.ONLINE)
        else:
            text = self.user.last_seen.toString(OFFLINE_FORMAT)
...
class SideBar(QFrame):
    def __init__(self, user, parent):
        ...
        self.contacts_button = MenuButton(
            self.parent(),
            icon="home/user-plus.svg",
            tip="New Contact",
            user=self.user,
            window=NewContactDialog,
        )
        self._layout.addWidget(self.contacts_button)

        self.groups_button = MenuButton(
            self.parent(),
            icon="home/users.svg",
            tip="New Group",
            user=self.user,
            window=NewGroupDialog,
        )
        self._layout.addWidget(self.groups_button)

        self.channels_button = MenuButton(
            self.parent(),
            icon="home/video-plus.svg",
            tip="New Channel",
            user=self.user,
            window=NewChannelDialog,
        )
        self._layout.addWidget(self.channels_button)

        self.setting_button = MenuButton(
            self.parent(),
...
class Header(QFrame):
    ...
    def set_online(self):
        ...
        self.status.setStyleSheet(
            "background: qlineargradient(spread:pad, x1:0.683616, y1:0.477, x2:1, y2:1, stop:0.587571 rgba(3, 146, 52, 255), stop:0.847458 rgba(255, 255, 255, 255));\ncolor: white;"
        )

    def set_offline(self):
        self.status.setText("OFFLINE")
        self.status.setStyleSheet(
            "background: qlineargradient(spread:pad, x1:0.585, y1:0.818136, x2:0.614, y2:1, stop:0.147727 red, stop:0.431818 rgba(146, 100, 136, 255), stop:0.880682 rgba(246, 184, 233, 255));\ncolor: white;"
        )
...
class Home(QFrame, Backend_Hook):
    ...
    def __init__(self, app, client=None, user=None):
    ...
    def setup(self):
        ...
        # center
        center, center_layout = SETUP_FRAME(
            mother_layout=hlayout, re_obj=1, space=5, margins=[5, 5, 5, 5], orient="h"
        )
        center.setStyleSheet("QFrame{background: white; border-radius: 10px}")

        # center view
        center_view, center_view_layout = SETUP_FRAME(
            mother_layout=center_layout, re_obj=1, space=10
        )
        ...
        self.channel_frame = CREATE_TAB(
            self.center_tab,
            "chat_list/user-group.svg",
            "Channels",
            ChatsList,
