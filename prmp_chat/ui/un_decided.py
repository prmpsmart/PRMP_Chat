...
class LineEditWithButton(QLineEdit):
    ...
    def __init__(
        ...
        completer=False,
        default_values=[],
        completer_sense=Qt.CaseInsensitive,
        callback=None,
    ):
        ...
        if completer or default_values:
            self.set_completer(default_values, completer_sense)
    ...
    def add_default_value(self, value):
...
class ChatObjectWidget(QFrame):
    ...
    def _style_me(self):
        self.setStyleSheet(
            "ChatObjectWidget{border-radius: 10px; background-color: #dadada;}ChatObjectWidget::hover{background-color: #a0a0a0;}"
        )

    def blink(self):
        self.setStyleSheet(
            "ChatObjectWidget{border-radius: 10px; background-color: #a0a0a0;}"
        )
    ...
    def render(self, alias, lastseen, access):
        ...
        self.piclabel.setStyleSheet(
            "border-radius: 25px; background-color: #f092de; font-size: 20px;"
        )
        layout2.addWidget(self.piclabel)
        self.namelabel = QLabel(alias)
        if access == 1:
            self.namelabel.setStyleSheet(
                "font-size: 20px; margin-left: 3px; color: #0f9000;"
            )
        elif access == 2:
            self.namelabel.setStyleSheet(
                "font-size: 20px; margin-left: 3px; color: #0020ff;"
            )
        else:
        ...
        if lastseen == "Online":
            self.seenlabel.setStyleSheet(
                "border-radius: 8px; background-color: #00f700;"
            )
        else:
            self.seenlabel.setStyleSheet(
                "border-radius: 8px; background-color: #fd0000;"
            )
...
class ChatRoomObjectWidget(QFrame):
    ...
    def adjust(self, message):
        ...
        if re.match(r"<img[^>]+title.*?>", message):
            pass
        elif length > 50:
            self.mlabel.setMinimumWidth(500)
        elif length > 40:
            self.mlabel.setMinimumWidth(400)
        elif length > 30:
            self.mlabel.setMinimumWidth(300)
        elif length > 20:
            self.mlabel.setMinimumWidth(200)
        elif length > 10:
            self.mlabel.setMinimumWidth(100)
        else:
            pass
    ...
    def _setup(self, sender="", message="", time=""):
        ...
        if time:
            ...
            self.tlabel.setStyleSheet(
                "font-size: 11px; color: black; background: %s" % self.background_color
            )
            self._layout.addWidget(self.tlabel)

        self.mlabel.setStyleSheet(
            "font-size: 14px; color: black; background: %s" % self.background_color
        )
    ...
    def createR(self, sender, message, time):
        self._setup(sender, message, time)

    def createS(self, message, time):
        self._setup(message=message, time=time)

    def createT(self, message):
        self._setup(message=message)

    def render(self, *args):
        if len(args) == 1:
            self.createT(args[0])
        elif len(args) == 2:
            self.createS(args[0], args[1])
        elif len(args) == 3:
            self.createR(args[0], args[1], args[2])
        else:
            return None
...
class ChatRoomWidget(QFrame):
    def __init__(self, receiver=None):
        ...
        self.setStyleSheet(
            u"QWidget {background:rgb(20, 36, 43); color: white}\n"
            "\n"
            " QPushButton{\n"
            "	border: 1px solid;\n"
            "	border-radius: 4px ;\n"
        ...
        embutton.setIcon(
            QIcon(
                os.path.join(
                    os.path.dirname(__file__), "resources", "images", "emoji.svg"
                )
            )
        )
        embutton.setIconSize(QSize(ss, ss))
        embutton.setToolTip("Open emoji picker")
        embutton.setStyleSheet(
            ".QPushButton{border: 2px solid #91b7b0; border-radius: %dpx; background-color: rgba(145, 163, 176, 200);}.QPushButton::hover{background-color: rgb(64, 64, 64);}"
            % (sy)
        )
        ...
        sebutton.setIcon(
            QIcon(
                os.path.join(
                    os.path.dirname(__file__), "resources", "images", "send.svg"
                )
            )
        )
        sebutton.setIconSize(QSize(ss, 32))
        sebutton.setToolTip("Send a message (Ctrl+Enter)")
        sebutton.setStyleSheet(
            ".QPushButton{border: 2px solid #08b7ff; border-radius: %dpx; background-color: rgba(8, 129, 255, 200);}.QPushButton::hover{background-color: rgb(8, 99, 255);}"
            % (sy)
        )
...
class ChatRoomsTab(QTabWidget):
    def __init__(self, parent=None):
        ...
        self.setStyleSheet(
            """QWidget {background:rgb(20, 36, 43)}; QTabWidget::pane{border: none;}"""
        )
        self.setStyleSheet(
...
class ChatObjectsWidget(QFrame):
    def __init__(self, callback=None):
        ...
        refresh_button.setIcon(
            QIcon(os.path.join("resources", "images", "refresh.svg"))
        )
        refresh_button.setIconSize(QSize(25, 25))
        refresh_button.setToolTip("Refresh participants list")
        refresh_button.setStyleSheet(
            ".QPushButton{border-radius: 15px;}.QPushButton::hover{background-color: rgba(94, 94, 94, 150);}"
        )
...
class AuthForm(QFrame):
    ...
    def create_frames(self):
        ...
        limage = QLabel()
        limage.setPixmap(
            QIcon(os.path.join("resources", "images", "chat.svg")).pixmap(QSize(64, 64))
        )
        ...
        self.tab_fr.setStyleSheet(
            """QTabBar::tab{color: #f1f1f1; width: 165px; height: 28px; font-size: 22px;}QTabBar::tab:!selected{background-color: #202020;}
