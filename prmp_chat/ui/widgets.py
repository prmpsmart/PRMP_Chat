...
HOME_STYLE = open(
    os.path.join(os.path.dirname(__file__), "resources/qss/normal.qss")
).read()
...
class MenuButton(Icon2Button):
    ...
    def action(self):
        if self._win:
            if not self._window:
                self._window = self._win(parent=self, user=self.user)
...
class CropHood(QLabel):
    ...
    def paintEvent(self, event):
        ...
        if self.shape in ["r", "s"]:
            qp.drawRect(self.selected)
        elif self.shape == "c":
            qp.drawEllipse(self.selected)
